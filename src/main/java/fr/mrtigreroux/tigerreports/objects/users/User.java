...
public class User {
    ...
    private static final String COOLDOWN_QUERY =
            "SELECT cooldown FROM tigerreports_users WHERE uuid = ?";
    private static final String NOTIFICATIONS_QUERY =
            "SELECT notifications FROM tigerreports_users WHERE uuid = ?";
    ...
    public User(UUID uuid, String lastDisplayName, UserData data) {
        ...
        this.lastDisplayName =
                lastDisplayName != null && !lastDisplayName.isEmpty() ? lastDisplayName : null;
    }
    ...
    public boolean addListener(UserListener listener, Database db, TaskScheduler taskScheduler,
            UsersManager um) {
        ...
        if (wasEmpty) { // Data is potentially expired or has never been collected
            LOGGER.info(
                    () -> getName() + ": addListener(" + listener
                            + "): updateDataOfUserWhenPossible(" + getName() + ")"
            );
    ...
    public boolean hasSameUserDataType(UserData otherUserData) {
        LOGGER.info(
                () -> getName() + ": hasSameUserDataType(" + otherUserData + "): "
    ...
    public String getDisplayName(VaultManager vm) {
        ...
        if (displayName != null && !displayName.isEmpty()) {
            ...
            LOGGER.debug(
                    () -> getName() + ": getDisplayName(): save lastDisplayName = "
                            + lastDisplayName
            );
        }
        ...
        if (lastDisplayName != null) {
            LOGGER.debug(
                    () -> getName() + ": getDisplayName(): return lastDisplayName = "
                            + lastDisplayName
            );
    ...
    public String getDisplayName(VaultManager vm, boolean staff) {
        if (UserUtils.useDisplayName(staff)) {
            LOGGER.debug(
                    () -> getName() + ": getDisplayName(staff = " + staff + "): return display name"
            );
    ...
    public void sendErrorMessage(String message) {
        ...
        if (p == null) {
            LOGGER.info(
                    () -> getName() + ": sendErrorMessage(" + message
                            + "): p = null, cannot send error message"
            );
            return;
        }
    ...
    public void sendReportNotification(Report r, boolean direct, Database db, VaultManager vm,
            BungeeManager bm) {
        if (!isOnline()) {
            return;
        }
    ...
    public void setImmunity(String immunity, boolean bungee, Database db, BungeeManager bm,
            UsersManager um) {
        ...
        if (!bungee) {
            if (bm != null) {
                bm.sendPluginNotificationToAll(
                        new ImmunityBungeeNotification(
                                bm.getNetworkCurrentTime(),
                                uuid,
                                this.immunity
                        )
                );
            ...
            db.updateAsynchronously(
                    "UPDATE tigerreports_users SET immunity = ? WHERE uuid = ?",
    ...
    public void startImmunity(boolean bungee, Database db, BungeeManager bm, UsersManager um) {
        setImmunity(
                DatetimeUtils.getRelativeDatetime(
                        ConfigFile.CONFIG.get().getLong("Config.ReportedImmunity", 120)
                ),
                bungee,
                db,
                bm,
    ...
    public void getImmunityAsynchronously(Database db, TaskScheduler taskScheduler, UsersManager um,
            BungeeManager bm, ResultCallback<String> resultCallback) {
        if (hasPermission(Permission.REPORT_EXEMPT)) {
            if (!IMMUNITY_ALWAYS.equals(immunity)) {
            ...
            if (immunity == null) {
                db.queryAsynchronously(
                        "SELECT immunity FROM tigerreports_users WHERE uuid = ?",
    ...
    public void setCooldown(String cooldown, boolean bungee, Database db, BungeeManager bm) {
        ...
        if (!bungee) {
            if (bm != null) {
                bm.sendPluginNotificationToAll(
                        new CooldownBungeeNotification(bm.getNetworkCurrentTime(), uuid, cooldown)
                );
            }
            db.updateAsynchronously(
                    "UPDATE tigerreports_users SET cooldown = ? WHERE uuid = ?",
    ...
    public void punish(long seconds, User staff, boolean bungee, Database db, BungeeManager bm,
            VaultManager vm) {
    ...
    public void stopCooldown(User staff, boolean bungee, Database db, BungeeManager bm) {
        ...
        if (staff != null) {
            ...
            MessageUtils.sendStaffMessage(
                    Message.STAFF_STOPCOOLDOWN.get()
                            .replace("_Player_", staff.getDisplayName(vm, true))
                            .replace("_Target_", getDisplayName(vm, false)),
                    ConfigSound.STAFF.get()
            );
            ...
            if (!bungee) {
                bm.sendPluginNotificationToAll(
                        new StopCooldownBungeeNotification(
                                bm.getNetworkCurrentTime(),
                                staff.getUniqueId(),
                                uuid
                        )
    ...
    public void changeStatistic(String statisticName, int relativeValue, boolean bungee,
            Database db, BungeeManager bm) {
        ...
        if (!bungee) {
            if (bm != null) {
                bm.sendPluginNotificationToAll(
                        new StatisticBungeeNotification(
                                bm.getNetworkCurrentTime(),
                                uuid,
                                statisticName,
                                relativeValue
            ...
            db.updateAsynchronously(
                    "UPDATE tigerreports_users SET `" + statisticName + "` = `" + statisticName
                            + "` + ? WHERE uuid = ?",
                    Arrays.asList(relativeValue, uuid.toString())
            );
        }
    ...
    public void getStatisticsAsynchronously(boolean useCache, Database db,
            TaskScheduler taskScheduler, ResultCallback<Map<String, Integer>> resultCallback) {
        ...
        public static SavedMessage from(String savedMessage) {
            ...
            if (savedMessage != null && savedMessage.length() >= datetimeLength) {
                return new SavedMessage(
                        savedMessage.substring(0, datetimeLength),
    ...
    public void openReportsMenu(int page, boolean sound, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        if (!isOnline()) {
            return;
        }
    ...
    public void openReportMenu(Report r, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        if (!isOnline()) {
            return;
        }
    ...
    public void openReportMenu(int reportId, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        if (!isOnline()) {
            return;
        }
    ...
    public void openProcessMenu(Report r, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        if (!isOnline()) {
            return;
        }
    ...
    public void openPunishmentMenu(int page, Report r, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        Player p = getPlayer();
        if (p == null) {
            return;
        ...
        if (command != null && !command.equalsIgnoreCase("none")) {
            r.process(
                    ...
                    Appreciation.TRUE,
                    false,
                    this.hasPermission(Permission.STAFF_ARCHIVE_AUTO),
                    true,
                    db,
                    rm,
                    vm,
            ...
            new PunishmentMenu(this, page, r.getId(), rm, db, taskScheduler, vm, bm, um)
                    .setReport(r)
                    .open(true);
        }
    }
    
    public void openConfirmationMenu(Report r, ConfirmationMenu.Action action, ReportsManager rm,
            Database db, TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm,
            UsersManager um) {
        if (!isOnline()) {
            return;
        }
        new ConfirmationMenu(this, r.getId(), action, rm, db, taskScheduler, vm, bm, um)
                .setReport(r)
                .open(true);
    }
    
    public void openCommentsMenu(int page, Report r, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, UsersManager um, BungeeManager bm, VaultManager vm) {
        if (!isOnline()) {
            return;
        }
        new CommentsMenu(this, page, r.getId(), rm, db, taskScheduler, um, bm, vm).setReport(r)
                .open(true);
    }
    ...
    public void openUserMenu(User tu, ReportsManager rm, Database db, TaskScheduler taskScheduler,
    ...
    public void openUserReportsMenu(User tu, int page, ReportsManager rm, Database db,
            TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm, UsersManager um) {
        if (!isOnline()) {
            return;
        }
    ...
    public void openUserAgainstArchivedReportsMenu(User tu, int page, ReportsManager rm,
            Database db, TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm,
            UsersManager um) {
        if (!isOnline()) {
            return;
        }
        new UserAgainstArchivedReportsMenu(this, page, tu, rm, db, taskScheduler, vm, bm, um)
                .open(true);
    }
    ...
    public void setOpenedMenu(Menu menu) {
        ...
        if (onData.openedMenu != null) {
            LOGGER.info(
                    () -> getName() + ": setOpenedMenu(): close previous menu " + onData.openedMenu
            );
    ...
    private void sendMessageWithReportButton(String message, Report r, String reportButtonMsg,
            String alertDetailsMsg) {
        if (!isOnline()) {
            return;
        }
        sendMessage(
    ...
    public void toggleCommentSentState(Comment c, Database db, TaskScheduler taskScheduler,
        ...
        if (isPrivate && isOnline()) {
            ...
            if (isPrivate && bungeeServerName != null) {
                bm.sendPluginNotificationToServer(
                        bungeeServerName,
                        new CommentBungeeNotification(
                                bm.getNetworkCurrentTime(),
                                reportId,
                                commentId,
                                getUniqueId()
    ...
    public void sendNotifications(ReportsManager rm, Database db, TaskScheduler taskScheduler,
                ...
                for (String notification : notifications) {
                    if (notification != null) {
                        if (notification.contains(COMMENT_NOTIFICATION_DATA_SEPARATOR)) {
                            String[] parts =
                                    notification.split(COMMENT_NOTIFICATION_DATA_SEPARATOR);
                            try {
                                rm.getReportByIdAsynchronously(
                                        ...
                                        new ResultCallback<Report>() {
                                            ...
                                            public void onResultReceived(Report r) {
                                                r.getCommentByIdAsynchronously(
    ...
    public void sendCommentNotification(Report r, Comment c, boolean direct, Database db,
            VaultManager vm, BungeeManager bm) {
        if (r == null || c == null) {
            return;
        }
        ...
        c.setStatus(
                ...
                db,
                TigerReports.getInstance().getReportsManager()
        );
    }
    ...
    public void cancelEditingComment() {
        ...
        sendMessageWithReportButton(
                Message.CANCEL_COMMENT.get().replace("_Report_", r.getName()),
                r
        );
        ...
        if (onData.editingComment.getId() == null) {
            r.addComment(this, commentMessage, db, taskScheduler, new ResultCallback<Integer>() {
                ...
                public void onResultReceived(Integer commentId) {
                    LOGGER.info(
                            () -> getName()
                                    + ": terminateEditingComment(): rm.updateDataWhenPossible()"
                    );
    ...
    public void teleportToReportParticipant(Report r, Report.ParticipantType targetType,
            boolean currentLocation, VaultManager vm, BungeeManager bm) {
        Player p = getPlayer();
        if (p == null) {
            return;
        }
        ...
        if (currentLocation) {
            if (t == null) {
                if (bm.isPlayerOnline(target)) {
                    ...
                    MessageUtils.sendErrorMessage(
                            p,
                            Message.PLAYER_OFFLINE.get().replace("_Player_", target)
                    );
                    return;
                }
            } else {
            ...
            if (loc == null) {
                MessageUtils.sendErrorMessage(
                        p,
                        Message.LOCATION_UNKNOWN.get().replace("_Player_", target)
                );
                return;
            }
        ...
        ConfigUtils.processCommands(
                ConfigFile.CONFIG.get(),
                "Config.AutoCommandsBeforeTeleportation",
                r,
                p,
                vm,
                bm
    ...
    public void processPunishing(Report r, String punishmentConfigPath,
            FileConfiguration configFile, ReportsManager rm, Database db,
    ...
    public void processPunishing(Report r, String staffReason, String punishmentConfigPath,
        ...
        afterProcessingAReport(
                ...
                vm,
                bm,
                TigerReports.getInstance().getUsersManager()
        );
        r.processWithPunishment(
                this,
                false,
                this.hasPermission(Permission.STAFF_ARCHIVE_AUTO),
                punishmentName,
    ...
    public void startProcessPunishingWithStaffReason(Report r, String punishmentConfigPath) {
        ...
        onData.pendingProcessPunishingData =
                new PendingProcessPunishingData(r, punishmentConfigPath);
        onData.p.closeInventory();
        String reportName = r.getName();
        sendMessageWithCancelButton(
                Message.PROCESS_REPORT_PUNISHING_WITH_STAFF_REASON.getPath(),
                reportName
        );
    }
    ...
    public void cancelProcessPunishingWithStaffReason() {
        ...
        sendMessageWithReportButton(
                Message.CANCEL_PROCESS_REPORT_PUNISHING_WITH_STAFF_REASON.get()
                        .replace("_Report_", r.getName()),
                r
        );
    ...
    public void terminateProcessPunishingWithStaffReason(String staffReason, ReportsManager rm,
            Database db, TaskScheduler taskScheduler, VaultManager vm, BungeeManager bm) {
        OnlineUserData onData = getOnlineUserData();
        if (onData == null) {
            return;
        ...
        processPunishing(
                onData.pendingProcessPunishingData.r,
                staffReason,
                onData.pendingProcessPunishingData.punishmentConfigPath,
                ConfigFile.CONFIG.get(),
                rm,
                db,
                taskScheduler,
     ...
     * 
     * @param r                     != null
     * @param allowAccessIfArchived if false, archived reports are considered not accessible (must
     *                              be restored to be accessed).
     *                              
     * @return
     */
    public boolean canAccessToReport(Report r, boolean allowAccessIfArchived) {
        ...
        boolean reportIsManagedByAnotherStaff = reportStatus == Status.IN_PROGRESS
                && r.getProcessingStaff() != null
                && !uuid.equals(r.getProcessingStaff().getUniqueId());
        if (
            (reportIsManagedByAnotherStaff
                    || reportStatus == Status.IMPORTANT
                    || reportStatus == Status.DONE) && !hasPermission(Permission.STAFF_ADVANCED)
        ) {
            return false;
        } else {
            return true;
    ...
    public void updateBasicData(Database db, BungeeManager bm, UsersManager um) {
        ...
        setImmunity(
                hasPermission(Permission.REPORT_EXEMPT) ? User.IMMUNITY_ALWAYS : null,
                false,
                db,
                bm,
                um
