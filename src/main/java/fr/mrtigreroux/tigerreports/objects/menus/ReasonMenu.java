...
public class ReasonMenu extends Menu {
    ...
    public Inventory onOpen() {
        Inventory inv =
                getInventory(Message.REASON_TITLE.get().replace("_Target_", tu.getName()), true);
        ...
        for (int reasonIndex = firstReason; reasonIndex <= firstReason + 26; reasonIndex++) {
            ...
            inv.setItem(
                    reasonIndex - firstReason + 18,
                    new CustomItem().fromConfig(configFile, path + ".Item").name(REASON_MESSAGE.replace("_Reason_", reason)).lore(REASON_DETAILS_MESSAGE.replace("_Player_", targetDisplayName).replace("_Reason_", reason).replace("_Lore_", MessageUtils.translateColorCodes(lore != null ? lore : "")).split(LINE_BREAK_SYMBOL)).hideFlags(true).create()
            );
        }
